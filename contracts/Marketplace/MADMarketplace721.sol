...
contract MADMarketplace721 is
    ...
    ERC721Holder
{
    using MarketplaceTypes for MarketplaceTypes.Order721;
    ...
    mapping(bytes32 => MarketplaceTypes.Order721) public orderInfo;
    ...
    function bid(bytes32 _order) public payable {
        MarketplaceTypes.Order721 storage order = orderInfo[_order];
    ...
    function _makeOrder(
        ...
        orderInfo[hash] = MarketplaceTypes.Order721(
            _id,
            _startPrice,
            _endPrice,
            block.timestamp,
    ...
    function _hash(IERC721 _token, uint256 _id, address _seller)
        internal
        view
        returns (bytes32)
    {
        ...
        returns (bool)
    {
        bytes memory encodedParams = abi.encodeWithSelector(
            IERC721.supportsInterface.selector, interfaceId
        );
        bool success;
        uint256 returnSize;
        uint256 returnValue;
    ...
    function _intPath(
        MarketplaceTypes.Order721 storage _order,
        uint256 _price,
        bytes32 _orderId,
        address _to,
        uint256 key
        ...
        returns (uint256 price)
    {
        ...
        assembly {
            ...
            case 1 {
                let _startPrice :=
                    and(sload(add(order.slot, 1)), shr(32, not(0)))
                let _startTime :=
                    and(sload(add(order.slot, 3)), shr(32, not(0)))
            ...
            case 2 {
                let lastBidPrice :=
                    and(sload(add(order.slot, 5)), shr(32, not(0)))

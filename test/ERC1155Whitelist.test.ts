  ...
  */
  ...
  describe("Free claim", async () => {
    ...
    it("Should gift tokens", async () => {
      ...
      const tx = await wl.giftTokens(gifted, Array.from(Array(10).keys()).map(_e=>1), 10);
      ...
      await expect(
        wl.connect(acc01).giftTokens(gifted, Array.from(Array(10).keys()).map(_e=>1), 10),
      ).to.be.revertedWith(WhitelistErrors.Unauthorized);
      await expect(
        wl.mintToCreator(10, Array.from(Array(10).keys()).map(_e=>1), 10),
  ...
  describe("Mint and batch mint to creator", async () => {
    it("Should mint to creator", async () => {
      const tx = await wl.mintToCreator(10, Array.from(Array(10).keys()).map(_e=>1), 10);
      ...
      // );
      await expect(
        wl.connect(acc01).mintToCreator(100, Array.from(Array(100).keys()).map(_e=>1), 100),
      ).to.be.revertedWith(WhitelistErrors.Unauthorized);
      await expect(
        wl.mintToCreator(10, Array.from(Array(10).keys()).map(_e=>1), 10),
      ).to.be.revertedWithCustomError(
    ...
    it("Should batch mint to creator", async () => {
      const tx = await wl.mintBatchToCreator([1], [1], 1);
      await wl.mintToCreator(9, Array.from(Array(9).keys()).map(_e=>1), 9);
      ...
      await expect(
        wl.connect(acc01).mintBatchToCreator([100], Array.from(Array(100).keys()).map(_e=>1), 100),
      ).to.be.revertedWith(WhitelistErrors.Unauthorized);
      await expect(
        wl.mintToCreator(1, [1], 1),
      ).to.be.revertedWithCustomError(
        wl,
        WhitelistErrors.MaxFreeReached,
      );
  ...
  describe("Burn", async () => {
    it("Should revert if not owner", async () => {
      const ids = [1];
      const tx = wl.connect(acc02).burn([acc02.address], ids, [1]);
    ...
    it("Should revert if id is already burnt/hasn't been minted", async () => {
      ...
      const tx = wl.connect(owner).burn([acc02.address, acc02.address, acc02.address], ids, [1, 1, 1]);
  ...
  describe("Batch burn", async () => {
    ...
    it("Should revert if id is already burnt/hasn't been minted", async () => {
      ...
      const tx = wl
        .connect(owner)
        .burnBatch(acc02.address, ids, [1, 1, 1, 1]);
    ...
    it("Should batch burn tokens, update storage and emit event", async () => {
      ...
      const ids1 = [1, 2];
      const ids2 = [3, 4];
      const tx1 = await wl.burnBatch(acc02.address, ids1, [1, 1]);
      const tx2 = await wl.burnBatch(acc01.address, ids2, [1, 1]);
      const bal1 = await wl.callStatic.balanceOf(
        acc02.address,
        1,
      );
    ...
    it("Should handle multiple batch burns", async () => {
      ...
      const tx1 = await wl.burnBatch(acc02.address, ids1, [1, 1, 1, 1, 1]);
      const tx2 = await wl.burnBatch(acc02.address, ids2, [1, 1, 1, 1, 1]);
      const tx3 = await wl.burnBatch(acc02.address, ids3, [1, 1, 1, 1, 1]);
      const tx4 = await wl.burnBatch(acc02.address, ids4, [1, 1, 1, 1, 1]);

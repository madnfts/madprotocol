...
describe("MADRouter1155", () => {
  ...
  describe("Burn", async () => {
    it("Should burn token for 1155Minimal collection type", async () => {
      ...
      await min.connect(acc01).publicMint(1, acc01.address, { value: price });
  ...
  describe("Batch Burn", async () => {
    ...
    it("Should batch burn token for 1155Basic collection type", async () => {
      ...
      await basic
        .connect(acc01)
        .mint(4, 1, acc01.address, { value: price.mul(pmul) });
      const tx = await r1155
        .connect(acc02)
  ...
  describe("Creator Withdraw", async () => {
    it("Should withdraw balance and ERC20 for all colTypes", async () => {
      ...
      await wl.connect(acc01).mint(1, [1], 1, acc01.address, { value: price });
  ...
  describe("Burn-setfees", async () => {
    it("Should burn token for 1155Minimal collection type", async () => {
      ...
      await r1155
        .connect(acc02)
        .setMintState(minAddr, true, 0);
      await min.connect(acc01).publicMint(1, acc02.address, { value: price });

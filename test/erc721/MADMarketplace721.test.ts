...
describe("MADMarketplace721", () => {
  ...
  describe("Fixed Price Listing", async () => {
    it("Should revert if transaction approval hasn't been set", async () => {
      ...
      await expect(
        ...
        basic,
        BasicErrors.TransferFromIncorrectOwner,
      );
      await expect(
        m721
          .connect(acc02)
        ...
        m721,
        MarketplaceErrors.NotOwnerNorApproved,
      );
    });
    it("Should revert if duration is less than min allowed", async () => {
      ...
      await expect(
        m721
          .connect(acc02)
          .fixedPrice(basic.address, 1, price, 0),
      ).to.be.revertedWithCustomError(
        m721,
    ...
    it("Should list fixed price order, update storage and emit event", async () => {
      ...
      expect(
        await m721.callStatic.tokenOrderLength(basicAddr, 1),
      ).to.eq(1);
      expect(
      ...
      await expect(fpTx)
        ...
        .withArgs(basicAddr, 1, fpOrderId, acc02.address);
    });
    it("Should handle multiple fixed price orders", async () => {
      ...
      // await r721
      //   .connect(acc02)
      //   .freeSettings(basicAddr, 1, 10, root);
      ...
      for (const wlOwner of Object.keys(wlOwners)) {
        await r721
          .connect(acc02)
          .basicMintTo(basic.address, wlOwner, 1, {
            value: ethers.utils.parseEther("0.25"),
          });
      }

      await basic.connect(mad).approve(m721.address, 1);
      await basic.connect(acc01).approve(m721.address, 2);
      await basic.connect(acc02).approve(m721.address, 3);
      await basic.connect(owner).approve(m721.address, 4);
      const tx = await basic
        .connect(amb)
      ...
      const owner1 = await basic.callStatic.ownerOf(1);
      const owner2 = await basic.callStatic.ownerOf(2);
      const owner3 = await basic.callStatic.ownerOf(3);
      const owner4 = await basic.callStatic.ownerOf(4);
      const owner5 = await basic.callStatic.ownerOf(5);
      ...
      await expect(fpTx1)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 1, fpOrderId1, mad.address);
      await expect(fpTx2)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 2, fpOrderId2, acc01.address);
      await expect(fpTx3)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 3, fpOrderId3, acc02.address);
      await expect(fpTx4)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 4, fpOrderId4, owner.address);
      await expect(fpTx5)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 5, fpOrderId5, amb.address);
    });
  });
  describe("Dutch Auction Listing", async () => {
    ...
    it("Should revert if startPrice is invalid", async () => {
      ...
      await expect(
        m721
          .connect(acc02)
          .dutchAuction(basic.address, 1, 3, 3, 700),
      ).to.be.revertedWithCustomError(
        m721,
        MarketplaceErrors.ExceedsMaxEP,
      );
    ...
    it("Should list dutch auction order, update storage and emit event", async () => {
      ...
      await expect(daTx)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 1, daOrderId, acc02.address);
    });
    it("Should handle multiple dutch auction orders", async () => {
      ...
      await expect(daTx1)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 1, daOrderId1, mad.address);
      await expect(daTx2)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 2, daOrderId2, acc01.address);
      await expect(daTx3)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 3, daOrderId3, acc02.address);
      await expect(daTx4)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 4, daOrderId4, owner.address);
      await expect(daTx5)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 5, daOrderId5, amb.address);
    });
  });
  describe("English Auction Listing", async () => {
    ...
    it("Should revert if duration is less than min allowed", async () => {
      ...
      await expect(
        m721
          .connect(acc02)
          .englishAuction(basic.address, 1, price, 300),
      ).to.be.revertedWithCustomError(
        m721,
        MarketplaceErrors.NeedMoreTime,
      );
    ...
    it("Should list english auction order, update storage and emit event", async () => {
      ...
      await expect(eaTx)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 1, eaOrderId, acc02.address);
    });
    it("Should handle multiple english auction orders", async () => {
      ...
      await expect(eaTx1)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 1, eaOrderId1, mad.address);
      await expect(eaTx2)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 2, eaOrderId2, acc01.address);
      await expect(eaTx3)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 3, eaOrderId3, acc02.address);
      await expect(eaTx4)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 4, eaOrderId4, owner.address);
      await expect(eaTx5)
        .to.emit(m721, "MakeOrder")
        .withArgs(basicAddr, 5, eaOrderId5, amb.address);
    });
  });
  ...
  describe("Buying", async () => {
    ...
    it("Should buy inhouse minted tokens, update storage and emit events", async () => {
      ...
      // dutch auction order
      const basicAddr2 =
        await f721.callStatic.getDeployedAddr(
          "BasicSalt2",
          acc02.address,
        );
      await f721
      ...
      const daOwnerOf = await basic2.callStatic.ownerOf(1);
  ...
  describe("Order Cancelling", async () => {
    ...
    it("Should cancel fixed price order", async () => {
      ...
      await expect(tx)
        ...
        .withArgs(basicAddr, 1, orderId, acc02.address);
  ...
  describe("Public Helpers", async () => {
    ...
    it("Should fetch the length of orderIds for a seller", async () => {
      ...
      await r721
        .connect(acc02)
        .basicMintTo(basicAddr, acc02.address, 3, {
          value: ethers.utils.parseEther("0.25"),
        });
      await basic.connect(acc02).approve(m721.address, 1);
      await basic.connect(acc02).approve(m721.address, 2);
      const tx_ = await basic
        .connect(acc02)
        .approve(m721.address, 3);
      const blockTimestamp = (

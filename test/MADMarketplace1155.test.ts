...
describe("MADMarketplace1155", () => {
  ...
  describe("Owner Functions", async () => {
    ...
    it("Should update marketplace settings", async () => {
      const tx = await m1155.updateSettings(600, 150, 40);
      ...
      await expect(tx)
        ...
        .withArgs(150, 600, 40);
      await expect(
        m1155.connect(acc01).updateSettings(300, 100, 40),
      ).to.be.revertedWith(MarketplaceErrors.Unauthorized);
    });
    ...
    it("Should delete order", async () => {
      ...
      const blocknum = m1155.provider.getBlockNumber();
      await r1155
        .connect(acc02)
  ...
  describe("Fixed Price Listing", async () => {
    it("Should revert if transaction approval hasn't been set", async () => {
      ...
      await expect(
        m1155
          .connect(acc01)
          .fixedPrice(min.address, 1, 1, price, 300),
    ...
    it("Should revert if duration is less than min allowed", async () => {
      ...
      await min
        .connect(acc02)
        .setApprovalForAll(m1155.address, true);
      const bn = await m1155.provider.getBlockNumber();
      ...
      await expect(
        m1155
          .connect(acc02)
          .fixedPrice(min.address, 1, 1, price, bn),
      ).to.be.revertedWithCustomError(
        m1155,
        MarketplaceErrors.NeedMoreTime,
      );
    ...
    it("Should revert if price is invalid", async () => {
      ...
      await expect(
        m1155
          .connect(acc02)
          .fixedPrice(min.address, 1, 1, 0, 300),
      ).to.be.revertedWithCustomError(
        m1155,
  ...
  describe("Dutch Auction Listing", async () => {
    it("Should revert if transaction approval hasn't been set", async () => {
      ...
      await expect(
        m1155
          .connect(acc01)
          .dutchAuction(min.address, 1, 1, price, 0, 300),
      ).to.be.revertedWith(MarketplaceErrors.NotAuthorized);
      await expect(
        m1155
          .connect(acc02)
    ...
    it("Should revert if duration is less than min allowed", async () => {
      ...
      await expect(
        m1155
          .connect(acc02)
          .dutchAuction(min.address, 1, 1, price, 0, bn),
      ).to.be.revertedWithCustomError(
        m1155,
        MarketplaceErrors.NeedMoreTime,
      );
    ...
    it("Should revert if startPrice is invalid", async () => {
      ...
      await expect(
        m1155
          .connect(acc02)
          .dutchAuction(min.address, 1, 1, 2, 3, 300),
      ).to.be.revertedWithCustomError(
        m1155,
      ...
      await expect(
        m1155
          .connect(acc02)
          .dutchAuction(min.address, 1, 1, 3, 3, 700),
      ).to.be.revertedWithCustomError(
        m1155,
        MarketplaceErrors.ExceedsMaxEP,
      );
    ...
    it("Should handle multiple dutch auction orders", async () => {
      ...
      const daTx1 = await m1155
        .connect(mad)
        .dutchAuction(whitelist.address, 1, 1, price, 0, 300);
  ...
  describe("English Auction Listing", async () => {
    it("Should revert if transaction approval hasn't been set", async () => {
      ...
      await expect(
        m1155
          .connect(acc01)
          .englishAuction(min.address, 1, 1, price, 300),
      ).to.be.revertedWith(MarketplaceErrors.NotAuthorized);
      await expect(
        m1155
          .connect(acc02)
    ...
    it("Should revert if duration is less than min allowed", async () => {
      ...
      await expect(
        m1155
          .connect(acc02)
          .englishAuction(min.address, 1, 1, price, bn),
      ).to.be.revertedWithCustomError(
        m1155,
        MarketplaceErrors.NeedMoreTime,
      );
    ...
    it("Should revert if startPrice is invalid", async () => {
      ...
      await expect(
        m1155
          .connect(acc02)
          .englishAuction(min.address, 1, 1, 0, 300),
      ).to.be.revertedWithCustomError(
        m1155,
        MarketplaceErrors.WrongPrice,
      );
  ...
  describe("Buying", async () => {
    ...
    it("Should revert if order is an English Auction", async () => {
      await m1155.updateSettings(20, 10, 20);
      await f1155.addAmbassador(amb.address);
      await f1155
        .connect(acc02)
        .splitterCheck("MADSplitter1", amb.address, 20);
    ...
    it("Should buy inhouse minted tokens, update storage and emit events", async () => {
      ...
      const daTx = await m1155
        .connect(acc02)
        .dutchAuction(basic.address, 1, 1, price, 0, 300);
      const daRc: ContractReceipt = await daTx.wait();
      const daBn = daRc.blockNumber;
      const daOrderId = getOrderId1155(
        daBn,
      ...
      // 201 - 33 = 168
      const dec = ethers.BigNumber.from(168).mul(tick);
    ...
    it("Should verify inhouse minted tokens balance changes", async () => {
      ...
      ).to.eq(fpRoyalty[1].add(daRoyalty[1]));
    });
    it("Should buy third party minted tokens with ERC2981 support", async () => {
      ...
      const fpTx = await m1155
        .connect(acc02)
        .fixedPrice(extToken.address, 1, 1, price, 300);
      const fpRc: ContractReceipt = await fpTx.wait();
      const fpBn = fpRc.blockNumber;
      const fpOrderId = getOrderId1155(
        fpBn,
      ...
      const daTx = await m1155
        .connect(acc02)
        .dutchAuction(extToken.address, 2, 1, price, 0, 300);
      const daRc: ContractReceipt = await daTx.wait();
      const daBn = daRc.blockNumber;
      const daOrderId = getOrderId1155(
        daBn,
      ...
      await expect(() =>
        ...
        [acc01, acc02, owner],
        ["-367647058823529432", daPrice.sub(daFee), daFee],
      );
    });
    it("Should buy third party minted tokens without ERC2981 support", async () => {
      ...
      const tick = price.div(delta);
      // 201 - 27 = 174
      const dec = ethers.BigNumber.from(174).mul(tick);
      const daPrice = price.sub(dec);
      ...
      await expect(() =>
        ...
        [acc01, acc02, owner],
        ["-362637362637362638", daPrice.sub(daFee), daFee],
      );
    });
  });
  describe("Claim", async () => {
    ...
    it("Should claim third party minted tokens with ERC2981 support", async () => {
      ...
      const eaTx = await m1155
        .connect(acc02)
        .englishAuction(extToken.address, 1, 1, price, 300);
      const eaRc: ContractReceipt = await eaTx.wait();
      const eaBn = eaRc.blockNumber;
      const orderId = getOrderId1155(
        eaBn,
  ...
  describe("Order Cancelling", async () => {
    ...
    it("Should revert due to already sold dutch auction order", async () => {
      ...
      const delta = ethers.BigNumber.from(300 - daBn);
      const tick = price.div(delta);
      const dec = ethers.BigNumber.from(184).mul(tick);
      const daPrice = price.sub(dec);

  ...
  */
  ...
  describe("Burn", async () => {
    it("Should revert if not owner", async () => {
      const ids = [1];
      const tx = basic.connect(acc02).burn([acc01.address], ids, [1]);
    ...
    it("Should revert if id is already burnt/hasn't been minted", async () => {
      ...
      const tx = basic.connect(owner).burn([acc02.address, acc02.address, acc02.address], ids, [1, 1, 1]);
    ...
    it("Should burn tokens, update storage and emit event", async () => {
      ...
      const tx2 = await basic.burn([acc01.address, acc01.address], [3, 4], [1, 1]);
  ...
  describe("Batch burn", async () => {
    ...
    it("Should batch burn tokens, update storage and emit event", async () => {
      ...
      const ids1 = [1, 2];
      const ids2 = [3, 4];
      const tx1 = await basic.burnBatch(acc02.address, ids1, [1, 1]);
      const tx2 = await basic.burnBatch(acc01.address, ids2, [1, 1]);
      const bal1 = await basic.callStatic.balanceOf(
        acc02.address,
        1,
      );
    ...
    it("Should handle multiple batch burns", async () => {
      ...
      const tx1 = await basic.burnBatch(acc02.address, ids1, [1, 1, 1, 1, 1]);
      const tx2 = await basic.burnBatch(acc02.address, ids2, [1, 1, 1, 1, 1]);
      const tx3 = await basic.burnBatch(acc02.address, ids3, [1, 1, 1, 1, 1]);
      const tx4 = await basic.burnBatch(acc02.address, ids4, [1, 1, 1, 1, 1]);

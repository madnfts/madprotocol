...
import { MarketplaceEventsAndErrors1155, IERC1155 } from "contracts/Shared/EventsAndErrors.sol";
...
contract MADMarketplace1155 is MADMarketplaceBase, MarketplaceEventsAndErrors1155, ERC1155Holder {
    ...
    mapping(IERC1155 => mapping(uint256 => mapping(uint256 => bytes32[]))) public orderIdByToken;
    ...
    mapping(uint256 => mapping(uint256 => mapping(uint256 => bool))) public feeSelector;
    ...
    function bid(bytes32 _order) external payable whenNotPaused {
        ...
        uint256 bidValue = address(erc20) != address(0) ? erc20.allowance(msg.sender, address(this)) : msg.value;
        ...
        emit Bid(order.token, order.tokenId, order.amount, _order, msg.sender, bidValue);
    }
    ...
    function buy(bytes32 _order) external payable whenNotPaused {
        ...
        if (
            !feeSelector[key][order.tokenId][order.amount] && MADFactory.creatorAuth(address(order.token), order.seller)
        ) {
    ...
    function _hash(IERC1155 _token, uint256 _id, uint256 _amount, address _seller) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(block.number, _token, _id, _amount, _seller));
    }
    ...
    function interfaceCheck(address account, bytes4 interfaceId) internal view returns (bool) {
        bytes memory encodedParams = abi.encodeWithSelector(IERC1155.supportsInterface.selector, interfaceId);
        bool success;
        uint256 returnSize;
        uint256 returnValue;
        assembly {
    ...
    function _intPath(
        ...
        emit Claim(_order.token, _order.tokenId, _order.amount, _orderId, _order.seller, _to, _price);
        // transfer token and emit event
        _order.token.safeTransferFrom(address(this), _to, _order.tokenId, _order.amount, "");
    }

    function _extPath0(Types.Order1155 storage _order, uint256 _price, bytes32 _orderId, address _to) internal {
    ...
    function _extPath1(Types.Order1155 storage _order, uint256 _price, bytes32 _orderId, address _to) internal {
        uint256 feePercent = maxFee; // _feeResolver(
        //     key,
        //     _order.tokenId,
        //     _order.amount
    ...
    function _feeResolver(uint256 _key, uint256 _tokenId, uint256 _amount) internal returns (uint256 _feePercent) {
        assembly {
            mstore(0x00, _key)
    ...
    function getCurrentPrice(bytes32 _order) public view returns (uint256 price) {
        ...
        assembly {
            ...
            case 1 {
                ...
                let _startTime := and(sload(add(order.slot, 4)), shr(32, not(0)))
    ...
    function tokenOrderLength(IERC1155 _token, uint256 _id, uint256 _amount) external view returns (uint256) {

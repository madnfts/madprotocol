  ...
  */
  ...
  describe("Batch mint", async () => {
    it("Should revert if supply has reached max", async () => {
      ...
      const id = [24];
      const amount = ethers.BigNumber.from(890);
      await wl.mint(890, { value: price.mul(amount) });
      const tx = wl
        .connect(mad)
  ...
  describe("Whitelist batch mint", async () => {
    ...
    it("Should revert if address is not whitelisted", async () => {
      ...
      const tx = wl
        .connect(acc01)
        .whitelistMintBatch([1], wrongProof, {
          value: price,
        });
  ...
  describe("Free claim", async () => {
    ...
    it("Should gift tokens", async () => {
      ...
      const tx = await wl.giftTokens(gifted);
  ...
  describe("Mint and batch mint to creator", async () => {
    it("Should mint to creator", async () => {
      const tx = await wl.mintToCreator(10);
    ...
    it("Should batch mint to creator", async () => {
      const tx = await wl.mintBatchToCreator([1]);
      await wl.mintToCreator(9);
  ...
  describe("Burn", async () => {
    it("Should revert if not owner", async () => {
      const ids = [1];
      const tx = wl.connect(acc02).burn(ids);
    ...
    it("Should revert if id is already burnt/hasn't been minted", async () => {
      ...
      const tx = wl.connect(owner).burn(ids);
  ...
  describe("Batch burn", async () => {
    it("Should revert if caller is not the owner", async () => {
      ...
      const ids = [1, 2, 3];
      await wl.setPublicMintState(true);
      await wl.mint(3, { value: price.mul(amount) });
      const tx = wl
        .connect(acc02)
    ...
    it("Should batch burn tokens, update storage and emit event", async () => {
      ...
      const ids1 = [1, 2];
      const ids2 = [3, 4];
      const tx1 = await wl.burnBatch(acc02.address, ids1);
      const tx2 = await wl.burnBatch(acc01.address, ids2);
      const bal1 = await wl.callStatic.balanceOf(
        acc02.address,
        1,
      );
    ...
    it("Should handle multiple batch burns", async () => {
      ...
      const tx1 = await wl.burnBatch(acc02.address, ids1);
      const tx2 = await wl.burnBatch(acc02.address, ids2);
      const tx3 = await wl.burnBatch(acc02.address, ids3);
      const tx4 = await wl.burnBatch(acc02.address, ids4);

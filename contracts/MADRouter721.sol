    ...
    ReentrancyGuard
{
    ...
    function whitelistSettings(
        ...
        // if(_tokenType < 2) revert ("INVALID_TYPE");
        if (_tokenType == 2) {
        ...
        whenNotPaused
    {
        ...
        _tokenType < 1 // require(ids.length == 1, "");
        ...
        whenNotPaused
    {
        ...
        // if (_tokenType > 2) {
        // ERC721Lazy(_token).mintToCreator(_amount);
        // }
        else revert("INVALID_TYPE");
    }
        ...
        returns (bytes32 colID, uint8 tokenType)
    {
        ...
        // return (colID, tokenType);
    }

    // function _minWithdrawERC20(address _token, ERC20 _erc20)
    //     internal
    ...
    //     }
    //     // (bool success,) =
    //         // _token.call(abi.encodeWithSignature("withdraw()")
    //     // );
    //     // if(!success) revert("WITHDRAW_FAILED");

    //     // assembly {
    //     //     let memP := mload(0x40)
    //     //     mstore(0x00, 0x3ccfd60b)
    //     //     if iszero(and(
    //     //         or(
    //     //             and(
    //     //                 eq(mload(0), 1),
    ...
    // function _withdrawERC20(address _token, ERC20 _erc20)
    // internal
    // {
    //     (bool success,) =
    //         _token.call(abi.encodeWithSignature("withdraw(ERC20)", _erc20)
    //     );
    //     if(!success) revert("WITHDRAW_FAILED");
    //     // ERC721Minimal(_token).withdrawERC20(_erc20)
    //     // assembly {
    //     //     let memP := mload(0x40)
    //     //     mstore(0x00, 0xe0d828bb)
    //     //     mstore(0x20, _erc20)
    ...
    //     //             iszero(returndatasize())
    //     //         ),
    //     //         call(gas(), _token, 0, 0x1c, 0x24, 0x00, 0x20)))
    //     //     {
    //     //         mstore(0x00, 0x750b219c)
    //     //         revert(0x1c, 0x04)
    //     //     }
    ...
    function _basicWithdraw(address _token)
        internal
        returns (bool success)
    {
        ERC721Basic(_token).withdraw();
    ...
    function _whitelistWithdraw(address _token)
        internal
        returns (bool success)
    {
        ERC721Whitelist(_token).withdraw();
    ...
    function _lazyWithdraw(address _token)
        internal
        returns (bool success)
    {
    ...
    function _stateType0(
        uint8 _tokenType,
        address _token,
        bool _state /* returns (bool success) */
    ) internal {
        if (_tokenType < 1) {
